...
sources: [raw/articles/2026-05-23-ai-agent-输出格式研究.md]
---
...
这一问题直接影响 [[agent-架构设计模式]] 中人机协作环节的设计——如果 Agent 的输出格式无法有效传达结构化信息，再好的编排模式也会在"最后一公里"失效。

## Markdown 的局限

Markdown 是「可浏览文本」的格式，其局限性在于：
...
这也印证了 [[anthropic-prompting-best-practices]] 中强调的：输出格式应服务于人类理解，而非仅满足模型生成便利。
...
这个闭环在纯 Markdown 模式下无法实现，也意味着输出格式的选择直接影响 [[agent-架构设计模式]] 中反馈回路的设计。

## 开放问题

- Markdown → HTML 的迁移成本与兼容性策略
- LLM 生成高质量 HTML 的可靠性与一致性
- 安全性：内嵌 JS 的沙箱隔离方案
- 与现有 Markdown 生态（Git、Wiki、Note-taking）的桥接
