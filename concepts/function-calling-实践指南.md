...
updated: 2026-05-23
type: concept
tags: [prompt-engineering, ai-agent, inference]
sources: [raw/articles/2026-05-23-function-calling-实践指南.md]
---
...
这一点与 [[ai-agent-输出格式研究]] 中讨论的结构性错配问题密切相关——LLM 输出的函数调用意图本质上是一种结构化输出，需要应用层正确解析和执行。
...
多轮工具调用是 [[hermes-agentic-workflows]] 中 delegate_task 模式的基础——子代理通过反复调用工具完成复杂任务，并将结果汇总返回主代理。
...
- [[ai-agent-输出格式研究]] — Function Calling 的输出是结构化数据，属于 Agent 输出格式设计的一部分
- [[hermes-agentic-workflows]] — Hermes 的 agentic 工作流深度依赖 Function Calling 实现工具调用和子代理编排
