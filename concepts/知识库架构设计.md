...
updated: 2026-05-23
type: concept
tags: [knowledge-mgmt, llm-wiki]
sources: [raw/articles/2026-05-23-知识库架构设计.md]
---

# 知识库架构设计

LLM Wiki 项目的知识库架构设计决策与演进历程，核心围绕「md 做记忆，html 做展示」的理念，确保内容的持久性、可追溯性和可恢复性。

## 设计理念

### md 做记忆，html 做展示

项目的核心架构理念，灵感来自「HTML 的不合理有效性」研究（参见 [[the-unreasonable-effectiveness-of-html]]）：
...
本项目同时采用手动策划（Wiki）和自动化（[[graphify]]）两种知识管理方式：

- **Wiki（手动）**：提供精心策划的页面、人工质量的交叉引用和来源追踪
- **Graphify（自动化）**：提供自动化语义连接和社区发现，参见 [[graphify]]
- **[[rag-检索增强生成]]**：知识库可作为 RAG 系统的外部知识源，通过文档切分和向量化实现语义检索

两者互补：Wiki 的手动 [[wikilinks]] 构成结构化骨架，Graphify 的自动化图谱发现隐藏的语义关联，[[rag-检索增强生成]] 则在此基础上实现智能问答。
